...
{
    private function makeController(PDO $Silian_pdo): LogSearchController
    {
        ...
        return new LogSearchController($Silian_pdo, $Silian_authService, $Silian_auditLogService);
    }
    ...
    public function testSearchFiltersAuditLlmAndErrorByConversationIdAndTurnNo(): void
    {
        ...
        $Silian_request = makeRequest('GET', '/admin/logs/search', null, [
            'types' => 'audit,error,llm',
            'conversation_id' => 'admin-ai-11111111',
            'turn_no' => '2',
        ]);
        $Silian_response = $Silian_controller->search($Silian_request, new Response());
        ...
        $this->assertCount(1, $Silian_payload['data']['audit']['items']);
        $this->assertSame('admin-ai-11111111', $Silian_payload['data']['audit']['items'][0]['conversation_id']);
        $this->assertSame('req-conv-1', $Silian_payload['data']['audit']['items'][0]['request_id']);

        $this->assertCount(1, $Silian_payload['data']['llm']['items']);
        $this->assertSame('admin-ai-11111111', $Silian_payload['data']['llm']['items'][0]['conversation_id']);
        $this->assertSame(2, $Silian_payload['data']['llm']['items'][0]['turn_no']);

        $this->assertCount(1, $Silian_payload['data']['error']['items']);
        $this->assertSame('req-conv-1', $Silian_payload['data']['error']['items'][0]['request_id']);
    }
    ...
    public function testSearchFiltersAuditByRequestIdAndReturnsRequestIdColumn(): void
    {
        ...
        $this->assertTrue($Silian_payload['success']);
        $this->assertCount(1, $Silian_payload['data']['audit']['items']);
        $this->assertSame('req-audit-1', $Silian_payload['data']['audit']['items'][0]['request_id']);
    }
}
